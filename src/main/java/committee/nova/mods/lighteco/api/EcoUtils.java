...
public class EcoUtils {
    ...
    public static EcoActionResult vary(
        ...
        if (NeoForge.EVENT_BUS.post(pre).isCanceled()) return EcoActionResult.EVENT_CANCELED;
        ...
        Optional.ofNullable(player.getCapability(ModCapability.ECONOMY)).ifPresent(a -> {
            ...
            NeoForge.EVENT_BUS.post(new BalanceVaryEvent.Post(player, baseValue, pastValue, newValue));
        });
        return result.get();
    }
    ...
    public static Optional<BigDecimal> getBalance(Player player) {
        ...
        Optional.ofNullable(player.getCapability(ModCapability.ECONOMY)).ifPresent(a -> result.set(a.getBalance()));
